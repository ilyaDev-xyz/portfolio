...
`public/` carries the icon family (`favicon.ico`, `favicon-32.png`, `apple-touch-icon.png`, `icon-192.png`, `icon-512.png`, `manifest.webmanifest`) plus the OG image set (`og-image.png`, `og/cases/<slug>.png` × 6 — committed sanitized demo content). All generated reproducibly from an external icon/OG pipeline (`_workspace/.archive/temp_scripts/`); no hand-edited assets.
...
For case routes the `vite.config.ts` plugin overrides `<head>` per slug, including `og:image` pointing at `/og/cases/<slug>.png`. Home and case OG are PNG, not SVG/WebP — LinkedIn rejects vector and WebP for `og:image`; PNG/JPG only.
...
The OG image (1200×630) mirrors the home Hero layout: warm ochre radial heat in the top half, tapered hairline + accent sheen at the divider, cool blue glow in the bottom half. All gradient stops match `tokens.css` exactly — the site palette is the single source of truth. Per-case OG reuses the same chrome with a left-aligned title, balanced-wrap subtitle, SCOPE/STACK meta rows under the divider, and an accent URL at bottom-right.
...
- `public/og-image.png` and `public/og/cases/<slug>.png` are committed and contain **public/demo content only**. They are safe to push to the public github repo via `npm run export:public` and serve on a public-content `build:public`.
- `npm run build:private` runs `scripts/overlay-private-og.mjs` after `vite build`, which copies the matching private masters from `_workspace/.archive/temp_scripts/out/` directly over `dist/og-image.png` and `dist/og/cases/<slug>.png`. `public/og/*` is never written by the private pipeline, so a stray `git status` after `build:private` cannot accidentally surface private OG bytes for commit.

Operator workflow when private content changes: regenerate masters in `_workspace/.archive/temp_scripts/` for both sources (`python gen_og.py && python gen_case_og.py`), copy public-set into `public/og/*` (`python gen_site_icons.py --source public`), commit the public diff. Then `npm run build:private` picks up the latest private masters via the overlay step. Set `SKIP_PRIVATE_OG_OVERLAY=1` in environments without private masters (CI build:private smoke checks).
...
Every page has a Markdown sibling at the same URL — `/cases/ai-crm.md`, `/index.md`, etc. RU gets `.ru.md`. Two corpora-level entry points sit at the root: `/llms.txt` (curated index per [llmstxt.org spec](https://llmstxt.org)), `/llms-full.txt` (everything concatenated, EN, single fetch).
...
Built at dev-server start and at `vite build` time from `en.ts` / `ru.ts` via two pure serializers (`src/lib/homeMarkdown.ts` for the home page, `src/lib/caseStudyMarkdown.ts` for cases). The same serializers power an in-page "Copy markdown" button (`src/components/CopyMarkdownButton.tsx`) — agents (and humans) can grab the page without scraping the rendered DOM.

Files are gitignored — they're build artefacts, never authoritative source. A direct-serve Vite middleware bypasses Vite's static handler for `.md` and `.txt` because Vite's SPA-fallback otherwise routes `.txt` requests through `index.html` (returning the SPA shell for `/llms.txt`). The middleware reads files from `publicDir` directly and pins `Content-Type: text/{plain,markdown}; charset=utf-8` — without the explicit charset browsers fall back to Windows-1252 and Cyrillic mangles to mojibake. Same fix needs an explicit `header` directive in Caddy on prod.

`llms-full.txt` carries size metadata on line 1 as an HTML comment (invisible in Markdown render, line-1 readable for raw-text agents) and in both language indexes' "Full corpus" link description. Format: `~Nk chars · ~N KB` — chars (string length, deterministic) are preferred over tokens (varies by tokenizer) per the spec's silence on size warnings; tokens are not added because they'd be misleading across cl100k vs o200k vs Claude tokenizer. Recomputed every build, never hand-edited. Filename remains canonical `llms-full.txt` — Cloudflare, Stripe and others use the same fixed path; renaming for size-warning purposes would break the convention agents look for.
