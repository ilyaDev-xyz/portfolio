...
describe('homeToMarkdown', () => {
  it('serialises EN home page (snapshot)', () => {
    expect(
      homeToMarkdown(en, ORIGIN, { mdExt: '.md', indexFile: 'llms.txt' }),
    ).toMatchSnapshot();
  });

  it('serialises RU home page (snapshot)', () => {
    expect(
      homeToMarkdown(ru, ORIGIN, { mdExt: '.ru.md', indexFile: 'llms-ru.txt' }),
    ).toMatchSnapshot();
  });

  it('starts with H1 of the hero name', () => {
    const md = homeToMarkdown(en, ORIGIN, { mdExt: '.md', indexFile: 'llms.txt' });
    expect(md.split('\n')[0]).toBe(`# ${en.hero.name}`);
  });
  ...
  it('honours mdExt for sibling case links', () => {
    const md = homeToMarkdown(ru, ORIGIN, { mdExt: '.ru.md', indexFile: 'llms-ru.txt' });
    expect(md).toContain(`${ORIGIN}/cases/${ru.projects[0].slug}.ru.md`);
    expect(md).not.toContain(`${ORIGIN}/cases/${ru.projects[0].slug}.md\n`);
  });
